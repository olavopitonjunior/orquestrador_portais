...
Mistério encerrado. A investigação da fundação registrou que **só o mysql2 (Node) autenticava; pymysql e mysql.connector davam `1045 Access denied`** para as mesmas credenciais, causa nunca explicada — e descartou cedo demais a hipótese da **codificação da senha**. Era exatamente ela.

**Causa raiz**: **senha não-ASCII**. Um caractere fora do ASCII ocupa 2 bytes em UTF-8 e 1 em latin-1. O hash `caching_sha2_password` no servidor foi criado sobre a forma **UTF-8**. O **pymysql (`connections.py`) força `.encode('latin1')` em senha `str`**, ignorando o `charset` — envia o byte errado, o hash não bate, `1045`. O mysql2 sempre funcionou por enviar UTF-8 nativo. **Não era falta de `cryptography` nem SSL.**
...
**Infra medida (31/08)**: MySQL **8.4.9**, `authentication_policy = *:caching_sha2_password` (`@@default_authentication_plugin` foi removido no 8.4). O transporte **não é forçado pelo servidor** — o pymysql auto-negocia TLS por padrão, e é ele quem garante a criptografia hoje. Confira a política de transporte antes de assumir TLS em qualquer cliente novo; o estado exato do servidor fica fora deste documento (D-012). A conta de leitura não tem `SELECT` em `mysql.user` (identidade e host ficam fora deste documento — D-012). Sanidade: `SELECT COUNT(*) FROM newcore_bi.FT_RealtyRelation` → 404.836 via pymysql.
