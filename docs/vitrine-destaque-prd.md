...
- Cerca de 44% do estoque elegível não possui avaliação por categoria em `realty_score_category_score` — **medido em 56,3 % entre os elegíveis da rodada 30417** (06/09/2026). É **compatível com a previsão** desta tabela de riscos, que dava como Alta a probabilidade de a penalidade "recair sobre metade dos candidatos". Ressalva de método: a medição **não separa** deriva do estoque de composição do conjunto elegível — entre os 48.812 candidatos a taxa é de 35,3 %, abaixo dos 44 % de 28/08, então quem produz os 56 % é o funil, e a definição de "elegível" mudou no período (a D-027 acrescentou a nona regra).
