...
- **Forma da normalização (nº 2) fixa no código**: min-max é a forma provisória escolhida (D-014, D-016 §3) — há uma única forma na v0, então ela é função no código (`_normalizar_minmax`), não um callable injetado (injetar uma forma sem segunda opção seria complexidade sem uso). O *valor* do parâmetro nº 2 segue nulo; o que está fixo é a forma escolhida, como as faixas de preço em `bucketizacao.py`. **Trocar a forma da normalização afeta o ranking e exige entrada no CHANGELOG.** Ambas rotuladas na planilha.
