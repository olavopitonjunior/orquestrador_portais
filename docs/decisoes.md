...
**Provisórios da rodada** (nº 2 forma da normalização; nº 3 intensidades das penalidades e decaimento) entram **injetados run-local** (`ParametrosDecisao`), rotulados na planilha, **nunca em `src/config` e nunca adotados** (D-014 mantém os dois nulos na lista canônica).
