...
A coluna da **composição da nota** chama-se `nota_bruta`, e não `nota_portal`: é o nome que o dado já tem no domínio e no Registro, e o grupo em que ela vive nesta tabela é justamente "Composição da nota", não "Sinais do portal". Ao lado dela vai `origem_da_nota`, também fora da lista acima, com três valores — `portal` quando a nota foi medida no anúncio do imóvel, `banco` quando a raspagem não entrou e a nota veio do desempate declarado, e `sem_anuncio` quando o portal entrou mas aquele imóvel não tinha anúncio e recebeu o valor imputado pelo tratamento declarado (`fim_da_fila`, D-034). Sem essa coluna a nota bruta é indistinguível entre as três procedências, e a limitação declarada na aba de parâmetros vale para a rodada inteira, não para a linha. Reconciliar a tabela normativa acima com o que a geração entrega continua pendente de decisão do dono.
...
|---|---|---|
| Nota do anúncio | 70 | único sinal com variância medida — 14 valores em 300 anúncios (03/09/2026), **69 em 55.162 na coleta completa (06/09)**, preenchimento de 100 %. Conclusão reforçada |
| Cliques, somados entre tipos (contato, telefone, WhatsApp, proposta, agendamento) | 30 | sinal fraco mas real, e é intenção de compra — **mais fraco do que a frase sugeria**: 1,69 % dos anúncios têm algum clique, e proposta e agendamento são zero em 55.162 de 55.162 ([P-25]) |
...
- A deriva dos números de referência foi medida, incorporada ao mapa de dados e **refletida no PRD em 06/09/2026** (D-036): o universo elegível é de 6.854 com as nove regras, contra 6.970 posições contratadas, e a causa é a queda de atividade de corretor por distrito, não mudança de regra. O que segue aberto é o **efeito** disso: com déficit no destaque, o relaxamento passa a ser o mecanismo ordinário de encher a cota, e o dono optou por mantê-lo assim (D-036).
