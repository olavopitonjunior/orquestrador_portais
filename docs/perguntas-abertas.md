...
| **[P-25]** — a alocação dos três pesos do portal | Os três somam 100, e a medição na coleta completa sugere que dois estão trocados: **cliques pesam 30 e distinguem 3,5 % dos pares de imóveis**; **visualizações pesam 0 e distinguem 38,6 %**. A nota, com 70, distingue 57,8 % — essa está confirmada. (Em não-zeros: cliques 124 dos 6.970 escolhidos, visualizações 1.550.) A justificativa dos cliques tinha duas metades: "sinal fraco mas real" (ficou muito mais fraca: 1,69 % dos anúncios, e dois dos cinco tipos são zero em 55.162 de 55.162) e "é intenção de compra, não curiosidade" (continua de pé). | **Nada trava**: os três valores adotados seguem vigentes e a rodada 30417 já rodou com eles. Mas 30 pontos de 100 estão num sinal que separa 1,8 % da lista, e o que separa 22 % não pesa nada. | `src/config/adotados.py` já tem a fiação — é trocar os números. A pergunta não é aritmética: é **quanto peso um sinal raro e forte merece contra um comum e fraco**, e isso é critério seu, não medição. |
...
**Fechadas em 2026-09-04 (D-027 a D-034)** — ficam aqui para a referência não apontar para o vazio: nº 3 (descontos em pontos de 100 e perdão por semana, D-030/D-034), nº 5 e nº 7 (idade máxima e cobertura mínima da raspagem, D-034), **[P-15]** (composição do sinal de portal, fechada pela D-028 — a razão escrita, "visualizações 0 em 300 de 300", **caiu em 06/09**: a coleta completa achou 23,9 % com visualizações; a pendência segue fechada, a justificativa não, ver [P-25]), **[P-16]** (desconto do perfil frágil — deixou de existir, D-027). Os nº 12 e nº 13 **deixaram de existir** sem receber valor (D-031): com o perfil como filtro e o portal como nota, a pergunta desapareceu.
...
| **nº 4** — tentativas e intervalo de repetição | Quantas vezes o sistema tenta de novo antes de desistir, e com que intervalo? | A "repetição antes de desistir" que justifica o LangGraph **não existe**. Prende também um defeito de concorrência registrado no `bug.md`. Desde 06/09 o raspador **classifica** a falha repetível (`TransientError` → `retryable` no status, e o console manda rodar de novo), mas não repete sozinho: falta o número, não o diagnóstico. | **O valor sozinho não destrava** o grafo: falta a política de repetição nos nós. Mas destrava a coleta externa, onde a classificação já espera por ele. |
...
| Id | Pergunta | Bloqueado hoje | Quando você responder |
|---|---|---|---|
| **[P-24]** | Por quantos dias um checkpoint de coleta completa interrompida ainda pode ser retomado? Reaproveita-se o nº 5 (2 dias, D-034) ou é parâmetro novo? | **Nada hoje** — a coleta completa nunca rodou em produção. Mas a guarda não existe: um checkpoint de semanas atrás retoma, e o CSV fica com duas gerações coladas sob um `finishedAt` de hoje, que a porta de idade aceita. | A guarda de idade entra em `podeRetomar`. Se preferir regra a número, cabe "só retoma no mesmo dia operacional" — e aí não há parâmetro. |
...
**Fechadas em 2026-09-05 (D-035)** — **[P-23]** foi autorizada pelo dono e executada: o PRD passou à versão 6.0, incorporando as D-001 a D-034 sem preencher parâmetro nulo nenhum e sem resolver nenhuma outra pergunta desta fila. **[P-18]** foi respondida dentro dela, e não como pedia: trocar "nove" por "oito" tornaria o PRD errado de um jeito novo, porque a D-027 fez o número voltar a nove por outro caminho — oito regras gerais mais o perfil de conversão. O que a revisão corrigiu foi a **composição** da tabela, que tinha dez linhas e passou a nove. A segunda tensão da linha :490 — sob o relaxamento, imóvel fora de até seis regras entra no destaque — **não** foi resolvida: o critério de aceite foi qualificado para dizer "não entra no ranking primário", o que o torna verificável sem contradizer a História 5.
...
| **[P-17]** *(ato)* | Tirar a credencial de produção do Newcore dos **arquivos de configuração de MCP**, onde ela está em texto claro, fora do fluxo do cofre. | **Encolheu em 02/09.** As duas metades que este item registrava caíram por medição: o campo da senha no cofre **funciona** (a conexão de leitura respondeu, 405.053 linhas), e `POSTGRES_URL` **não precisa** estar no cofre — a forma de socket local não carrega usuário, senha nem host, logo não há credencial a proteger. O `op inject` falhava por **três referências mortas no próprio template**, já corrigidas; duas eram do Canal Pro e contradiziam a D-010. O diagnóstico medido está no adendo do [P-17] em `docs/decisoes.md`. O que resta é só a cópia em texto claro fora deste repositório — que a rotação recusada (**D-026**) torna permanente. | A credencial deixa de existir em cópia sem dono. |
...
| **[P-13]** *(fato)* | Quem aplica a carga encontra o imóvel pelo código do portal que a apuração agora entrega (`codigo_portal`), ou precisa de outra referência? | **Mudou em 04/09.** Estava refutada no banco; a primeira raspagem real mostrou `realties.NewIdMarketingRotation` igual ao `codigoImovel` do Canal Pro em 300 de 300, e o `apuracao.csv` já o traz. Falta só você confirmar que é esse o código que a pessoa usa. Adendo em `docs/decisoes.md`. |
...
- **No super destaque**, o PRD fixa o objetivo de ranking sobre "disputa real — mais de dez candidatos por posição". Essa frase **deixou de ser verdadeira**: são 7,5. É justamente o nível onde a nota do portal (D-028) e o filtro de perfil (D-027) mais decidem quem entra, e onde o invariante 7 proíbe relaxar para compensar.

**Confirmado em 2026-09-06, e é pior do que parecia.** A contagem foi repetida noutro dia, e a medição de 02/09 deixou de ser única: o pipeline devolveu 7.801 (02/09), 8.230 (04/09) e 8.197 (06/09) — patamar de 7.801 a 8.230 ao longo de cinco dias, 5,3% de amplitude em torno da média, contra os 10.290 de 28/08, que ficam 25% acima do topo dessa faixa. É **degrau, não oscilação diária**. E a causa está medida, na mesma coluna e com o mesmo predicado: **os distritos com dois ou mais corretores produtivos caíram de 61 (28/08) para 45 em 02/09 e 46 em 06/09** (com três ou mais, de 39 para 18) — a queda aconteceu antes de 02/09 e depois achatou, que é o que faz disto um degrau e não uma rampa, e 21,4% dos imóveis ativos estão hoje em distrito sem nenhum. As três primeiras etapas do funil batem a 0,3% nas duas datas, e a única mudança de predicado no período mede −12 elegíveis — **não é a régua que mudou, é a atividade comercial que caiu**.

**O número que muda a sua decisão:** com as nove regras, o universo elegível ficou **na borda do contrato** — 7.009 em 05/09 às 23h52 (39 acima das 6.970 posições) e **6.854** em 06/09 às 08h35 (**116 abaixo**). Houve, portanto, pelo menos um instante medido abaixo da cota. Os 155 imóveis entre as duas leituras vieram do degrau do distrito, e a causa observável é a reconstrução do mart de BI entre elas (`FT_Districts` atualizada às 08h17) — não a rolagem das janelas: o corte da única regra que consome a data mudou em um imóvel. Duas leituras não estabelecem frequência, e são de sábado e domingo; o único ponto de sexta da série deu o maior valor. O que está estabelecido é que **não há margem confortável**, e que quando o estoque não enche, a cota do destaque fecha cedendo o perfil — primeiro degrau da cedência. O relaxamento **deixou de ser plano B**.

**Incorporado**: o detalhe completo — série do pipeline, cortes por regra, distribuição de corretores por distrito e a sensibilidade ao mínimo — está em [`mapa-de-dados.md`](mapa-de-dados.md), seção "O funil pelo pipeline". O PRD 6.0 declara a deriva como ressalva datada (D-035). **O que era seu, e você respondeu em 06/09** (logo abaixo): decidir se a disputa de "dez candidatos por vaga" no super destaque, que fundou o objetivo de ranking daquele nível, ainda sustentava o desenho — critério, não medição.

**Respondido em 2026-09-06 (D-036).** Você optou por **manter o desenho**: quando faltar imóvel aprovado, encher as posições cedendo o perfil, em vez de deixar vaga vazia. Com isso o PRD, a Spec e o `CLAUDE.md` passaram a publicar os números medidos — 6.854 elegíveis, 2.389 candidatos ao super (5,0 por vaga), 6.379 para o destaque (déficit de 116). No super destaque a premissa dobrou mas não quebrou; no destaque ela quebrou, e a consequência é que o relaxamento deixou de ser plano B.

**Recomendação minha, que só vira compromisso se você quiser:** acompanhar o número de posições preenchidas por **cedência** nas primeiras semanas de operação real. Se a maior parte da cota passar a encher cedendo o perfil — a regra que a D-027 acabou de criar —, valeria reabrir a pergunta, aí com dado de operação em vez de projeção. Você não pediu isso; fica como sugestão.
