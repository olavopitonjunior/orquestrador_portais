...
| **[P-15]** — composição do sinal de desempenho de portal | Como combinar nota do anúncio, visualizações e cliques num sinal só? | O runner usa `visualizações` como default run-local, **não adotado**. Muda o desempenho de portal de todo imóvel. **Medido em 03/09/2026 (primeira raspagem real, 300 anúncios): `visualizacoes` = 0 em 300 de 300 e cliques quase todos 0 — a forma default não tem sinal nesta API; a `nota` (LQS) tem 14 valores distintos. Recomendação: `forma = "nota"` (com `quando_ausente`). Ver `bug.md`.** | Escolha registrada + CHANGELOG. Já é injetada, não está fixa no domínio. |
