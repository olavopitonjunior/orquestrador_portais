...
| **[P-16]** — desconto do perfil frágil | Quanto vale a menos um perfil sustentado por poucos casos? A Spec §6.2 diz "não recebe peso pleno" e não quantifica. | Tunável provisório, injetado por rodada. Fica **fora** dos quatorze parâmetros e sem prazo. | Valor adotado; a fiação já existe. |
...
| **[P-17]** *(ato, urgente)* | Consertar o acesso ao banco: as credenciais do Newcore no 1Password são **recusadas** pelo servidor, e `POSTGRES_URL` **não existe** no item. | Bloqueia medir contra a base — logo o passo 5 da verificação contra a spec e a recontagem que a incorporação da deriva exige. E sem `POSTGRES_URL`, nada ligado ao Registro roda: aprovação, janelas, console. |
