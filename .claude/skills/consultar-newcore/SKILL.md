...
4. **Vagas**: ausente de `realties` e da tabela central. Localização em investigação — não assuma que existe.

Os demais: campos de placa e impulsionamento integralmente vazios; `adswhitelist` abandonada desde 2022; `adsblacklist` viva (12.155 imóveis) mas decidida como ignorada; ~44% do estoque elegível sem avaliação em `realty_score_category_score` (imóvel sem avaliação passa e recebe penalidade, não é excluído).
