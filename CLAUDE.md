...
Os dois níveis têm problemas de sinal oposto e por isso objetivos de ranking distintos. No super destaque há disputa real — 2.389 candidatos para 475 vagas, cinco por posição (06/09/2026) — e o ranking persegue valor esperado (probabilidade de conversão ponderada pelo ticket). No destaque **não há mais folga**: 6.379 candidatos para 6.495 vagas, déficit de 116. O objetivo segue sendo não deixar benefício contratado sem uso — o ranking persegue probabilidade de gerar lead, e regras podem ser relaxadas para preencher posições —, mas o relaxamento deixou de ser plano B e passa a ser o mecanismo ordinário de encher a cota, cedendo primeiro o filtro de perfil. Desenho mantido pelo dono na **D-036**. Números medidos pelo pipeline; a série e a causa da queda estão em `docs/mapa-de-dados.md`.
...
- **Automação de navegador** para a raspagem do Canal Pro, por **CDP em Chrome real** (`puppeteer-core` como transporte, não como piloto): anexa a um Chrome que o operador autenticou, captura os cabeçalhos de auth que a SPA do painel usa e daí em diante chama a API do portal de dentro da própria página. **Não lê DOM e não usa seletores de dado.** Selenium foi descartado por propriedade da técnica — o Turnstile detecta chromedriver (D-010). O login é sempre manual, e o re-aquecimento após bloqueio também.
- **Caminho de modelo na raspagem: NÃO existe.** A Ferramentas §2 previa "determinístico por seletores primeiro, modelo interpretando a página quando falha"; a **D-010** removeu isso do Coletor Externo v0 em 31/08, e em 2026-09-06 o dono **declinou reintroduzi-lo** (**D-037**). No desenho atual não há página a interpretar — esta é conclusão do levantamento, não fala dele. Se um dia for reintroduzido, a D-010 já fixou a obrigação: nenhum payload do painel vai a modelo sem remoção de identidades antes do envio (invariante 3).
- **Um provedor comercial de modelo por API** — ainda não escolhido ([P-14]), e **hoje nenhum agente usa modelo**: não há SDK de modelo no repositório, o Analista de Perfil é contagem determinística e o Redator é template. Os dois pontos onde um modelo caberia por desenho são o Analista de Perfil e o resumo da rodada do Redator.
...
- **Elegibilidade**: conjunto de **oito** regras eliminatórias gerais, binárias e sem compensação, aplicadas antes do ranking, **mais o perfil de conversão como nona regra desde a D-027** (2026-09-04: só entra quem casa um perfil robusto que contenha a faixa de preço); reprovar em uma basta para excluir. O piso de R$ 700.000 é condição de nível do super destaque aplicada na alocação, e o status impeditivo é regra de saída imediata — não regras de elegibilidade (decisões D-002 e D-003; a Spec 1.1 e o **PRD 6.0**, ambos revistos em 2026-09-05, já listam as nove assim — oito gerais mais o perfil; D-035).
- **Perfil de conversão**: combinação de uma ou duas características de imóvel observada nas vendas assinadas do período (janela adotada: 180 dias), sempre acompanhada do número de casos que a sustenta. Desde a **D-027** é REGRA, não fator: conta para o filtro o perfil **robusto** (N ≥ 3, D-014) que **contém a faixa de preço**; perfil frágil não conta. O "perfil que puxou" — o de mais vendas entre os que o imóvel casa — é rótulo da justificativa. Spec 1.1 §6.2.
