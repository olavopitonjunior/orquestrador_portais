...
## As colunas que saíram da planilha ficaram sem explicação nas rodadas já entregues

**Data**: 2026-09-07 · **Severidade**: média (o operador perde a explicação de coluna em três rodadas entregues, uma delas aguardando aprovação; nenhum número muda) · **Onde**: `console/app/rodada/[id]/page.tsx` e `console/lib/glossario-da-planilha.ts`

- **Esperado**: quem abre uma rodada entregue passa o mouse no cabeçalho e lê o que aquela coluna significa. A própria página promete isso: "Passe o mouse no cabeçalho para ver o que cada coluna significa."
- **Ocorrido**: a página lê o ARTEFATO em disco, e artefato é registro histórico — a planilha entregue mantém para sempre os nomes de coluna que tinha quando foi gerada. Ao remover `perfil_fragil` (fatia 1 da issue #73) e renomear `nota_portal`/`portal_pesou` (fatia 3), as chaves saíram do glossário `SOBRE_A_COLUNA` e o tooltip sumiu das colunas que **todas as rodadas em disco têm**. E não eram três: `semelhanca_perfil` e `desempenho_proprio` estavam no mesmo estado desde `da58897` (05/09), quando o desenho de quatro fatores caiu — a instância mais antiga é **anterior** à issue #73. Medido por arquivo em `saida/sexta/`: `perfil_fragil` 7, `semelhanca_perfil` 5, `desempenho_proprio` 5, `nota_portal` 3, `portal_pesou` 2.
- **Como apareceu**: o dono pediu para abrir o console no navegador. Não apareceu em nenhuma das duas revisões de código nem no meu próprio levantamento — os três raciocinaram sobre o artefato que o sistema PASSARÁ a gerar. O artefato que já existe não aparece em diff nenhum.
- **Afetou carga publicada?**: não. O tooltip é leitura de tela; a carga é aplicada a partir da planilha, não do console.
- **Situação**: **resolvido em 2026-09-07.** As cinco chaves voltam, cada uma dizendo que é nome ANTIGO ou coluna REMOVIDA, datando o corte (são dois: 05/09 e 06/09) e apontando a sucessora. O glossário sai de `page.tsx` para módulo próprio com teste — terceira vez nesta série que lógica presa na página escapa da rede, depois de `abas-da-rodada.ts` e `portao-da-coleta.ts`.

**A lição, e ela é a continuação exata da entrada logo acima.** Aquela dizia: *enumerar a função não basta; é preciso enumerar o dado*. Esta desce mais um nível: **enumerar o dado em memória não basta; é preciso enumerar o dado já GRAVADO EM DISCO.** Um artefato entregue é contrato com o passado, e remover a chave que o explica quebra a leitura do que já foi entregue — sem quebrar teste nenhum, porque nenhum teste olhava para lá.

**E a guarda que eu escrevi para isso nasceu TAUTOLÓGICA**, que é a parte que mais vale guardar. Ela varria as planilhas em disco, montava o conjunto das colunas sem tooltip — e então perguntava apenas sobre as chaves que outro teste já garantia existirem. Não podia ficar vermelha sozinha. A prova de que não protegia é que ela **passou verde** com `semelhanca_perfil` e `desempenho_proprio` órfãs no disco, ali, no mesmo diretório que ela acabara de ler. Uma guarda que lê a evidência certa e depois joga fora a pergunta que a justificaria é pior que guarda nenhuma: ela dá por coberto o que não está. Reescrita para afirmar que **toda** coluna que a página RENDERIZA COM TOOLTIP ou está explicada ou está declarada como dispensada, com a lista das dispensadas MEDIDA e não adivinhada; hoje fica vermelha sozinha quando uma órfã aparece. Achado do `orchestrator` no portão, antes do commit.

**E ainda assim a correção nasceu curta uma segunda vez**, o que fecha o argumento melhor que qualquer conclusão minha. A guarda reescrita varria `apuracao` — que a página mostra como CARD, não como tabela — e ignorava `relaxamento`, `perfis` e `parametros_e_limitacoes`, que ela renderiza. **Dez colunas estavam órfãs na tela naquele instante**, e a guarda passava verde: a revisão provou injetando uma coluna falsa numa dessas abas. A causa é a mesma das três vezes anteriores — uma lista própria em vez da fonte de verdade que já existia (`ORDEM_DAS_ABAS`), agora importada. E a whitelist de dispensadas, que eu declarara "medida", trazia 26 nomes dos quais 19 só existem numa aba que não renderiza tooltip: whitelist que ninguém confere silencia coluna nova, então encolheu para 7 e ganhou teste contra inflar.
